...
RUN apt-get update && apt-get dist-upgrade -y && apt-get install -y libpq-dev
...
# Set the DBHOST environment variable to host.docker.internal
ENV DBHOST host.docker.internal

# Command to start Swift application
CMD [ "sh", "-c", "cd /swift-project && .build-ubuntu/release/EmojiJournalServer" ]
